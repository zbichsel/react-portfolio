export const divWrapper = "antialiased font-nav py-6 md:py-16 mx-0.5 md:mx-[2rem] lg:mx-[5rem] xl:mx-[2rem]";

// !===!

export const divWrapper2 = "text-center";

// !===!

export const divWrapper3 = "xs:mx-3 md:mx-8 px-8 grid gap-8 grid-cols-1 md:grid-cols-2 md:px-12 lg:px-16 xl:px-32 py-16 mx-auto border bg-[#2a1e48] border-zinc-950 text-gray-900 rounded-xl shadow-sm shadow-black";

// !===!

export const divWrapper4 = "flex flex-col justify-between";

// !===!

export const header = "text-4xl lg:text-5xl text-center md:text-left font-bold leading-tight text-zinc-50";

// !===!

export const header2 = "mt-2 text-xl lg:text-5xl text-center md:text-left leading-tight text-zinc-50";

// !===!

export const divWrapper6 = "mt-8 text-center";

// !===!

export const span = "uppercase text-sm text-zinc-50 font-semibold";

// !===!

export const input1 = "w-full bg-zinc-900 text-zinc-50 placeholder-zinc-50 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline";

// !===!

export const textrea = "w-full h-32 bg-zinc-900 text-zinc-50 placeholder-zinc-50 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline ";

// !===!

export const input2 = "w-full bg-zinc-900 text-zinc-50 placeholder-zinc-50 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline";

// !===!

export const btn = "text-[#D242CF] uppercase hover:text-zinc-50 border border-[#D242CF] hover:bg-[#D242CF] focus:ring-4 focus:outline-none focus:ring-pink-300 font-medium leading-snug rounded-lg text-md px-5 py-2.5 text-center mr-2 mb-2 w-full";

// !===!

export const imgWrp = "mx-auto md:m-0 pt-2 pl-1 pb-1 pr-0 rounded-lg flex justify-center items-center shadow-md h-56 w-56 bg-[#2a1e48] border border-zinc-900/50";

// !===!

export const lepre = "w-48 rounded-lg shadow-lg cursor-default";